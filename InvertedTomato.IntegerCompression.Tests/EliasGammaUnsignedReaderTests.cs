...
namespace InvertedTomato.IntegerCompression.Tests {
    ...
    public class EliasGammaUnsignedReaderTests {
        private ulong Read(string input) {
            return EliasGammaUnsignedReader.ReadOneDefault(ByteArrayUtility.ParseBinaryString(input));
        }
        ...
        public void Read_15_15_15() {
            using (var stream = new MemoryStream(ByteArrayUtility.ParseBinaryString("00001000 00000100 00000010 00000000"))) {
                using (var reader = new EliasGammaUnsignedReader(stream)) {
