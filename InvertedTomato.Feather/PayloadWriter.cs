...
namespace InvertedTomato.Feather {
    public class PayloadWriter : Payload {
        ...
        public PayloadWriter AppendToSeconds(TimeSpan value) {
            var rawValue = (int)value.TotalSeconds;
            Inner.Write(rawValue);
            return this; // Allow chaining
        }
        public PayloadWriter AppendNullableToSeconds(TimeSpan? value) {
            if (!value.HasValue) {
                Inner.Write((byte)0x00);
                return this; // Allow chaining
            }
            ...
            Inner.Write((byte)0x01);
            return AppendToSeconds(value.Value); // Allow chaining
        }
        [Obsolete("Use AppendToSeconds() instead. This method definition will change in a future release.")]
        public PayloadWriter Append(TimeSpan value) { return AppendToSeconds(value); }
        [Obsolete("Use AppendNullableToSeconds() instead. This method definition will change in a future release.")]
        public PayloadWriter AppendNullable(TimeSpan? value) { return AppendNullableToSeconds(value); }
